...
trait MapInstances {
  ...
  ): Keys[mutable.Map[K, V], K, V, F] =
    new Keys[mutable.Map[K, V], K, V, F] {
  ...
  implicit def mapGet[K, V, F[_]](
      implicit Pure: Pure[F]
  ): Get[mutable.Map[K, V], K, V, F] =
    new Get[mutable.Map[K, V], K, V, F] {
      override def get(map: mutable.Map[K, V], key: K): F[Option[V]] =
        Pure.pure(map.get(key))
    }

  implicit def mapPut[K, V, F[_]](
      implicit Pure: Pure[F]
  ): Put[mutable.Map[K, V], K, V, F] =
    new Put[mutable.Map[K, V], K, V, F] {
      override def put(map: mutable.Map[K, V], key: K, value: V): F[Boolean] = {
  ...
  implicit def mapRemove[K, V, F[_]](
    ...
    new Remove[mutable.Map[K, V], K, V, F] {
