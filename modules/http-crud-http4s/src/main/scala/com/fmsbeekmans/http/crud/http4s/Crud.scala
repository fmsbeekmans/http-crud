...
case class Crud[Backend, K, V, F[_]](
  ...
  val browse: Browse[Backend, K, V, F] = Browse(backend, path)
  val create: Create[Backend, K, V, F] = Create(backend, path)
  val read: Read[Backend, K, V, F] = Read(backend, path)
  val update: Update[Backend, K, V, F] = Update(backend, path)
  val delete: Delete[Backend, K, V, F] = Delete(backend, path)
